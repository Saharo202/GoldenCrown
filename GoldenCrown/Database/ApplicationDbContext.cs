...
namespace GoldenCrown.Database
{
    public class ApplicationDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            accountEntity.Property(x => x.Balance).HasColumnName("balance").IsRequired();
            accountEntity.HasOne<User>().WithOne().HasForeignKey<Account>(x => x.UserId).OnDelete(DeleteBehavior.Cascade);

            var sessionEntity = modelBuilder.Entity<Session>().ToTable("sessions");
            sessionEntity.HasKey(x => x.UserId);
            sessionEntity.Property(x => x.UserId).HasColumnName("userd_id").IsRequired();
            ...
            transactionEntity.Property(x => x.Amount).HasColumnName("amount").IsRequired();
